...
# https://stackoverflow.com/questions/46702987/python-pygame-randomly-draw-non-overlapping-circles
...
# bouncing other circle
def move_circle_other(a,b):
    dist_cir = int(math.hypot(a.x - b.x, a.y - b.y))
    two_cir_radius = a.r + b.r
    if dist_cir == two_cir_radius:
        a.move_x *= -1
        a.move_y *= -1
        b.move_x *= -1
        b.move_y *= -1
...
def move_circle_frame():
    ...
    for n in range(len(obj_cir)):
        ...
        #for j in range(len(obj_cir)-1):
        #    if i != j :
        #        dist_other = int(math.hypot(obj_cir[i].x - obj_cir[j].x, obj_cir[i].y - obj_cir[j].y))
        #        sum_radius = obj_cir[j].r + obj_cir[j].r
        #    
        #        if dist_other <= sum_radius:
        #            if  obj_cir[i].x < obj_cir[j].x :
        #                obj_cir[i].move_x *= -1
        #                obj_cir[j].move_x *= -1
        #            if  obj_cir[i].y < obj_cir[j].y :
        #                obj_cir[i].move_y *= -1
        #                obj_cir[j].move_y *= -1
