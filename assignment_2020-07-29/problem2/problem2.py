                ...
                for j in range(N):
                    if ( i*rw <= mouse_x1 <= (i+1)*rw  ):
                        if ( j*rh <= mouse_y1 <= (j+1)*rh ) :
                            rect1 = rect_list.index((i*rw, j*rh, rw, rh))  # เช็คว่ากด mouse ที่ภาพไหน (หา index ของภาพใน rect_list)
                ...
                for j in range(N):
                    if ( i*rw <= mouse_x2 <= (i+1)*rw  ):
                        if ( j*rh <= mouse_y2 <= (j+1)*rh ) :
                            rect2 = rect_list.index((i*rw, j*rh, rw, rh))  # เช็คว่าปล่อย mouse ที่ภาพไหน (หา index ของภาพใน rect_list)
                            #สลับภาพ 2 ตำแหน่ง
                            temp = rect_img[rect1]
